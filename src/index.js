...
const createProductCard = ({ id, photoUrl, name, price }) => {
  ...
  productCard.innerHTML = `
      <article class="store__product product">
          ...
          <button type="button" class="product_btn-add-cart" data-id="${id}">Заказать</button>
...
const submitOrder = async (event) => {
  ...
  try {
    const response = await fetch(`${API_URL}/api/orders`, {method: 'POST', headers: {
    ...
    orderMessageText.textContent = `Ваш заказ оформлен, номер ${orderId}. Вы можете его забрать завтра после 10 AM`;
